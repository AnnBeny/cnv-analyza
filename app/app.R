...
server <- function(input, output, session) {
  ...
  #  #codes <- substr(base_names, nchar(base_names) - 1, nchar(base_names)) # nolint
      ...
      selected_cols_list <- lapply(seq_along(file_list), function(i) {
        tryCatch({
          ...
          base_name <- tools::file_path_sans_ext(
            gsub("(_cov\\.txt|[ _]*\\(paired\\)[ _]*Targ(et|ed)?[ _]*Region[ _]*Coverage\\.txt)$", "", filenames[i])
          )
  ...
  output$downloadCoverage <- downloadHandler(
    ...
    filename = function() { 
      paste0("coverage_", format(Sys.time(), "%Y%m%d"), ".csv") 
    },
  ...
  output$downloadCNVM <- downloadHandler(
    ...
    filename = function() { 
      paste0("CNV_M_", format(Sys.time(), "%Y%m%d"), ".csv") 
    },
    content = function(file) {
  ...
  output$downloadCNVZ <- downloadHandler(
    ...
    filename = function() { 
      paste0("CNV_Z_", format(Sys.time(), "%Y%m%d"), ".csv") 
    },
    content = function(file) {
